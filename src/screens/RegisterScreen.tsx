...
export default function RegisterScreen({ onSwitchToLogin }: RegisterScreenProps) {
  ...
  const handleRegister = async () => {
    ...
    setLoading(true);
    try {
      const res = await signUpWithEmail(fullName, email, pass, confirmPass);
      if (res.error) {
        setError(res.error);
      } else {
        // Đăng ký thành công, Supabase tự động đăng nhập hoặc yêu cầu xác nhận email
      }
    } catch (err) {
      setError('Có lỗi xảy ra. Vui lòng thử đăng ký lại.');
    } finally {
      setLoading(false);
    }
  };
        ...
        >
          ...
          <View style={styles.card}>
            ...
            <View style={styles.headerContainer}>
              ...
              <Text style={styles.subtitle}>Đăng ký để bắt đầu hành trình tiết kiệm cùng Capy</Text>
            </View>
